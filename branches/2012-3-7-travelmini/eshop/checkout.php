...
if (!function_exists('eshopShowform')) {
	...
	if($eshopoptions['shipping']=='4' && 'no' == $eshopoptions['downloads_only'] && !eshop_only_downloads()){
		...
		foreach ($typearr as $k=>$type){
			...
			if($query['0']->maxweight!='' && $cartweight > $query['0']->maxweight)
				continue;
			$eshopshiptableinner ='
			<table class="eshopshiprates eshop thisTable" summary="'.__('Shipping rates per mode','eshop').'">
			
			<tbody class="thisTbody">';
			...
			foreach ($query as $row){
				...
				for($z=1;$z<=$eshopoptions['numb_shipzones'];$z++){
					$y='zone'.$z;
					$eshopshiptableinner.='<td headers="'.$eshopletter.$y.'" class="'.$y.' thisMoney">'.sprintf( __('%1$s%2$s','eshop'), $currsymbol, $row->$y).'</td>';
				}
			...
			if($row->area=='country')
				...
				$eshopshiptableheadtext = sprintf( __('%1$s <small>%2$s</small>','eshop'),stripslashes(esc_attr($type)), __('','eshop'));
	...
	if(eshop_discount_codes_check()){
		...
		$echo .='<fieldset class="eshop fld5"><legend><label for="eshop_discount" id="discount_label">'.__('Discount Code','eshop').'</label></legend>
