...
function cp_logout_url( $url = '' ) {
  ...
  if( is_user_logged_in() ) :
    ...
    if( isset($_COOKIE['fbsr_'.get_option($app_abbr.'_fb_appid')]) )
      return $facebook->getLogoutUrl( array( 'next' => wp_logout_url($url) ) );
    else
...
if ( !function_exists('cp_ad_loop_thumbnail') ) :
    ...
    $image_id = cp_get_featured_image_id($post->ID);
		...
		if ( get_option('cp_ad_image_preview') == 'yes' )
		    $prevclass = 'preview child';
		else
		    $prevclass = 'nopreview child';
...
function cp_timeleft($theTime) {
    ...
    $expired_label = __('Advertiser, activate your ad here','appthemes');
    ...
    return '<a href="http://advertiser.expert.com.sg" title="'.$theText.'">'.$theText.'</a>';
}
...
function cp_get_popular_ads(){
    ...
    $end = $start + $posts_per_page + 1; // Add an additional entry to test for a next page

    // give us the most popular ads based on page views
    $sql = $wpdb->prepare( "SELECT SQL_CALC_FOUND_ROWS p.* FROM " . $wpdb->prefix . "cp_ad_pop_total a "
