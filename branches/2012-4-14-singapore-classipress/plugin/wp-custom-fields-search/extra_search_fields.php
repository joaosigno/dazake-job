...
class DB_Search_Widget extends DB_WP_Widget {
	...
	function sql_restrict($where){
		if($this->isPosted()){
			...
			//$where = preg_replace("/AND $wpdb->posts. = '(post|page)'/","",$where);
...
function CheckBoxField($options=array(),$params=array()){
	...
	function getInput($name,$joiner,$fieldName=null){
		...
		foreach($this->getOptions($joiner,$fieldName) as $option=>$label){
			...
			$options.="<div class='check-box-wrapper'><input type='checkbox' name='$id' id='$htmlId' value='$option'$checked> <label for='$htmlId'>$label</label></div>";
		}
		return $options;
	}
	function getCSSClass(){
	...
	function getConfigForm($id,$values){
		return "<label for='$id-checkbox-options'>Check Box Options</label><input id='$id-checkbox-options' name='$id"."[checkboxoptions]' value='$values[checkboxoptions]'/>";
	}
}
