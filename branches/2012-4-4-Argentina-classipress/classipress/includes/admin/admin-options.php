...
function cp_exclude_cats ($id = NULL) {
    ...
    if ( $id )
        $sql = $wpdb->prepare( "SELECT form_cats FROM ". $wpdb->prefix ."cp_ad_forms WHERE id != %s", $id );
    else
        $sql = $wpdb->prepare( "SELECT form_cats FROM ". $wpdb->prefix ."cp_ad_forms" );
...
appthemes_add_submenu_page_content(); // do_action hook
...
function cp_dashboard() {
   ...
   global $app_twitter_rss_feed, $app_forum_rss_feed, $options_dashboard;
	...
	$customers_today = $wpdb->get_var("SELECT COUNT(ID) FROM $wpdb->users INNER JOIN $wpdb->usermeta ON $wpdb->users.ID = $wpdb->usermeta.user_id WHERE $wpdb->usermeta.meta_key = 'wp_capabilities' AND ($wpdb->usermeta.meta_value LIKE '%contributor%') AND $wpdb->users.user_registered >= '$date_today'");
	$customers_yesterday = $wpdb->get_var("SELECT COUNT(ID) FROM $wpdb->users INNER JOIN $wpdb->usermeta ON $wpdb->users.ID = $wpdb->usermeta.user_id WHERE $wpdb->usermeta.meta_key = 'wp_capabilities' AND ($wpdb->usermeta.meta_value LIKE '%contributor%') AND $wpdb->users.user_registered BETWEEN '$date_yesterday' AND '$date_today'");
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function() {
			<?php if ( !function_exists('curl_init') ) : ?>
			    var fbErrMsg = '<?php esc_js( _e("Your server does not have curl enabled so Facebook login will not work. Contact your host provider to have it enabled.", "appthemes") ); ?>';
			    jQuery('#tab4 table tbody').html('<tr><td colspan="2"><span style="color:red">'+ fbErrMsg +'</td></tr>');
			<?php endif; ?>
...
function cp_ad_packs() {
    ...
    ?>
        ...
        if ( isset($_POST['submitted']) ) {
			...
			$results = $wpdb->insert( $wpdb->prefix . "cp_ad_packs", $values);
    ...
    ?>
	    ...
	    $where = array(
            ...
            $wpdb->update( $wpdb->prefix . "cp_ad_packs", $values, $where);
    ...
    case 'delete':
        ...
        $wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "cp_ad_packs WHERE pack_id = %s", $_GET['id'] ) );
    ?>
    ...
    default:
        ...
        $results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "cp_ad_packs ORDER BY pack_id desc" ) );
...
function cp_coupons() {
    ...
    case 'delete':
        ...
        $wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "cp_coupons WHERE coupon_id = %s", $_GET['id'] ) );
    ?>
...
function cp_form_layouts() {
    ...
    ?>
	    ...
	    // @todo Change to Insert
            $insert = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "cp_ad_forms" .
    ...
    ?>
	    ...
	    // @todo Change to Update
            $update = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "cp_ad_forms SET" .
                            " form_label    = %s," .
                            " form_desc     = %s," .
                            " form_cats     = %s," .
                            " form_status   = %s," .
    ...
    */
    ...
    ?>
		...
		// @todo Change to Insert
                $insert = $wpdb->prepare( "INSERT INTO " . $wpdb->prefix .
                        "cp_ad_meta (form_id, field_id) VALUES ( %s, %s)",
        ...
        if ( $theswitch ) $wpdb->query( $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "cp_ad_meta WHERE meta_id = %s", $_GET['del_id'] ) );
    ...
    default:
        ...
        $results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "cp_ad_forms ORDER BY id desc" ) );
...
function cp_custom_fields() {
    ...
    ?>
        ...
        if ( isset( $_POST['submitted'] ) ) {
            ...
            $insert = "INSERT INTO " . $wpdb->prefix . "cp_ad_fields ( field_name, field_label, field_desc, field_tooltip, field_type, field_values, field_search, field_owner, field_created, field_modified ) VALUES ( '" .
    ...
    ?>
	    ...
	    // @todo Change to Update
            $update = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "cp_ad_fields SET" .
                    " field_name = %s," .
                    " field_label = %s," .
                    " field_desc = %s," .
                    " field_tooltip = %s," .
    ...
    case 'delete':
        ...
        if ( !$results->field_perm > 0 ) {
            ...
            $delete = "DELETE FROM " . $wpdb->prefix . "cp_ad_fields WHERE field_id = '". $_GET['id'] ."'";
			...
			<div id="tab0">
                ...
                -->
                    ...
                    <thead>
                        ...
                        <thead>
                        <tr>
                            <th scope="col" width="200px"><?php _e('Facebook Check','appthemes')?></th>
                            <th scope="col">&nbsp;</th>
                        </tr>
                    </thead>
                        ...
                        <tr>
                            ...
                            <td class="forminp"><?php if ( function_exists('curl_init') ) echo '<span style="color:green">' . __('Your server has curl enabled.', 'appthemes'). '</span>'; else echo '<span style="color:red">' . __('Your server does not have curl enabled so Facebook login will not work. Contact your host provider to have it enabled.', 'appthemes'). '</span>'; ?></td>
                        </tr>

                        <tr>
                            <td class="titledesc"><?php _e('JSON DECODE Check','appthemes')?></td>
                            <td class="forminp"><?php if ( function_exists('json_decode') ) echo '<span style="color:green">' . __('Your server has json_decode enabled.', 'appthemes'). '</span>'; else echo '<span style="color:red">' . __('Your server does not have json_decode enabled so Facebook login will not work. Contact your host provider to have it enabled.', 'appthemes'). '</span>'; ?></td>
                        </tr>
