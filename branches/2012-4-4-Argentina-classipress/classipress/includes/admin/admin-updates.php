...
function cp_update_advanced_search_db() {
	...
	?>
        ...
        <p class="info"><?php _e( 'Geocoding ad listing addresses to enable the advanced search radius feature to work. This process queries Google Maps to get longitude and latitude coordinates based on each ad listings address. Please be patient as this may take a few minutes to complete.', 'appthemes' ); ?></p>
        <?php
...
function cp_upgrade_317() {
	...
	$sql = $wpdb->prepare( "SELECT field_name FROM ". $wpdb->prefix . "cp_ad_fields WHERE field_type = 'checkbox' " );
