...
if ( !function_exists('cp_show_refine_search') ) :
    function cp_show_refine_search($catid) {
        ...
        if ( $results ) :
            ...
            if ( $results )
                echo cp_refine_search_builder($results,$catid); // loop through the custom form fields and display them 
				...
				<form method="post" name="refine-search">
					...
					?>
						...
						<li class="distance">
							...
							<input name="<?php echo esc_attr( $fname ); ?>" id="<?php echo esc_attr( $fname ); ?>" type="text" minlength="2" value="<?php if(isset($_POST[$fname])) echo esc_attr( $_POST[$fname] ); ?>" class="text" />
							<div class="clr"></div>
					...
					foreach ( $locarray as $value ) :
						...
						if ( $value->field_name == 'cp_price' ) {
							...
							<li class="amount">
								<label class="title"><?php echo $value->field_label; ?>:</label>
...
?>
	...
	<li class="<?php echo esc_attr( $name ); ?>">
		<label class="title"><?php echo $label; ?></label>
		...
		<div class="element">
			...
			<div class="scrollbox">
				...
				<ol class="checkboxes">
					...
					<?php
					$cur = isset( $_POST[$name] ) ? $_POST[$name] : array();
...
function custom_search_refine_where($where) {
    ...
    if ( is_search() && isset($_GET['s']) && isset($_POST['refine_search']) ) {
      $query = '';
