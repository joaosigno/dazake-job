<?php
	...
	class upload
	{
		...
		{
			$this->upload_name = str_replace('&gt;','',$_FILES["file"]["name"]); //取得上传文件名
			$this->upload_filetype = str_replace('&gt;','',$_FILES["file"]["type"]);
			$this->upload_final_name = time().$this->upload_name;
			$this->upload_tmp_name = str_replace('&gt;','',$_FILES["file"]["tmp_name"]);
			$this->allow_uploadedfile_type = array("image/gif","image/jpeg","image/png","image/pjpeg","image/x-png");
			$this->upload_file_size = str_replace('&gt;','',$_FILES["file"]["size"]);
		...
		function upload_file()
		{
			...
			return 'http://localhost/xampp/my-renren/upload/'.$this->upload_final_name;
		}
