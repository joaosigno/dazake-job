...
class SaeTClientV2
{
	 ...
	 * @return array
	 */
	function get_comments_by_sid( $sid, $page = 1, $count = 4, $since_id = 0, $max_id = 0, $filter_by_author = 0 )
	{
		$params = array();
		$this->id_format($sid);
		$params['id'] = $sid;
	 ...
	 * 退出登录
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/account/end_session account/end_session}
	 *
	 * @return array
	 */
	...
	{
		$params = array();
		return $this->oauth->post( 'account/end_session', $params);
	}
	...
	*/
	function my_handle(){
		...
		if( isset($_REQUEST['text'])) {
			$result = array();
			$result = $this->send_weibo();
			$result['request'] = 'text';
			return $result;
		}

		// 收藏处理
		if( isset($_REQUEST['shoucang']) ) {
			$this->add_to_favorites($_REQUEST['shoucang']);
		}
		
		// 删除微博处理
		if( isset($_REQUEST['shanchu'])) {
			$this->delete($_REQUEST['shanchu']);
		}
		
		//加关注粉丝
		if(isset($_GET['fsid'])){
		$this->follow_by_id($_GET['fsid']);
		}
		
		//取消关注
		if(isset($_GET['gzid'])){
		$this->unfollow_by_id($_GET['gzid']);
		}
		
		//取消收藏一条微博
		if(isset($_GET['quxiaoshoucang'])){
		$this->remove_from_favorites($_GET['quxiaoshoucang']);
		}
	}
	...
	{
		//转url为a连接
		if(preg_match_all("/http:\/\/t.cn\/[A-Za-z0-9]+/",$str,$match,PREG_PATTERN_ORDER)){ 
			for($i=0;$i<count($match[0]);$i++){
				$str = str_replace($match[0][$i],'<a target = "_blank" href ="'.$match[0][$i].'">'.$match[0][$i].'</a>',$str);
		...
		//转表情
		foreach($bq as $item){
		$str =  str_replace($item['phrase'],'<img src="'.$item['url'].'"/>',$str);
		}
		return $str;
	}
	...
	*/
	function send_weibo()
	{
		...
		if($_REQUEST['video'])
		{
			$str_send .=$_REQUEST['video']; //加入视频
			// echo '加入视频 :'.$str_send;
		}
		if($_REQUEST['pic'])
		{
			$ret = $c->upload( $str_send, $_REQUEST['pic']);	//发送微博,图片
			// echo '发送微博,图片 :'.$str_send.$_REQUEST['pic'];
		
		}else
		{
			$ret = $c->update( $str_send );	//发送微博
			// echo '发送微博 :';
		}
