...
include_once(TEMPLATEPATH.'/includes/theme-facebook.php');
...
if ( !function_exists('cp_ad_loop_thumbnail') ) :
	function cp_ad_loop_thumbnail() {
		...
		if ( $images ) {
			...
			$adthumbarray = wp_get_attachment_image( $image->ID, 'ad-small' );
                    ...
                    foreach ($results as $result) :
                        ...
                        if (!empty($post_meta_val))
                            if($result->field_type == "checkbox"){
                                ...
                                echo '<li id="'. $result->field_name .'"><span>' . $result->field_label . ':</span> ' . appthemes_make_clickable(implode(", ", $post_meta_val)) .'</li>'; // make_clickable is a WP function that auto hyperlinks urls}
        ...
        if (in_array($result->field_name, $custom_field_keys) || ($result->field_name == 'post_content') || ($result->field_name == 'post_title') || ($result->field_name == 'tags_input') || $result->field_type == 'checkbox' ) :
            ...
            ?>
                <li id="list_<?php echo $result->field_name; ?>">
                    ...
                    <input name="<?php esc_attr_e($result->field_name); ?>" id="<?php esc_attr_e($result->field_name); ?>" type="text" class="text<?php if ($result->field_req) echo ' required'; ?>" style="min-width:200px;" value="<?php if ($result->field_name == 'post_title') { esc_attr_e($getad->post_title); } elseif ($result->field_name == 'tags_input') { echo rtrim(trim(cp_get_the_term_list($getad->ID, APP_TAX_TAG)), ','); } else { esc_attr_e($post_meta_val); } ?>" />
                    <div class="clr"></div>
                </li>
            <?php
            break;
            ...
            ?>
				<li id="list_<?php echo $result->field_name; ?>">
					<div class="labelwrapper">
						<label><?php if ($result->field_tooltip) : ?><a href="#" tip="<?php esc_attr_e($result->field_tooltip); ?>" tabindex="999"><div class="helpico"></div></a><?php endif; ?><?php esc_html_e($result->field_label);?>: <?php if ($result->field_req) echo '<span class="colour">*</span>' ?></label><br />
                        <label class="invalid" for="<?php esc_attr_e($result->field_name); if(stristr($result->field_name, 'checkbox')) echo '_list' ?>"><?php _e('This field is required.','appthemes');?></label>
					</div>
...
class cp_CategoryDropdown extends Walker {
    ...
    function start_el(&$output, $category, $depth, $args) {
        ...
        $output .= $pad.$cat_name;
        $output .= ' - ' . get_option($app_abbr.'_curr_pay_type_symbol') . get_option('cp_cat_price_'.$category->cat_ID) . '</option>'."\n";;
    }
}
...
if (!function_exists('cp_cat_menu_drop_down')) {
    function cp_cat_menu_drop_down($cols = 3, $subs = 0) {
      ...
      $cp_cat_nocatstext = get_option('cp_cat_strip_nocatstext');

			// get all cats for the taxonomy ad_cat
			$cats = get_terms(APP_TAX_CAT, 'hide_empty=0&hierarchical=1&pad_counts=1&show_count=1&orderby='.$cp_cat_orderby.'&order=ASC');
      $subcats = array();
      ...
      if ( count($cats) > 0 ){
            ...
            unset($cats[$key]);
  			...
  			foreach($cats as $cat) :
  				...
  				$cp_cat_menu .= '<ul>';
  				$cp_cat_menu .= '<li class="maincat cat-item-'. $cat->term_id .'"><a href="'. get_term_link( $cat, APP_TAX_CAT ) .'" title="'. esc_attr( $cat->description ) .'">'. $cat->name .'</a> '.$show_count.'</li>';


  				// don't show any sub cats
  				if($subs <> 999) :
            $cat_children = get_term_children( $cat->term_id, APP_TAX_CAT );
            $subs_count = 0;
            if ( count($subcats) > 0 ){
                ...
                if($cp_cat_hide_empty == 1 && $subcat->count == 0)
                  continue;
                ...
                if($subs_count >= $subs && $subs != 0)
                  continue;
        				...
        				// only show the total count if option is set
        				if($cp_cat_child_count == 1)
        					$show_child_count = '('. $subcat->count .')';
                else
                  $show_child_count = '';
          
        				$cp_cat_menu .= '<li class="cat-item cat-item-'. $subcat->term_id .'"><a href="'. get_term_link( $subcat, APP_TAX_CAT ) .'" title="'. esc_attr( $subcat->description ) .'">'. $subcat->name .'</a> '.$show_child_count.'</li>';
                $subs_count++;
              }
            }
            if($cp_cat_nocatstext != '' && $subs_count == 0)
              $cp_cat_menu .= '<li>'.__('No categories', 'appthemes').'</li>';
...
if (!function_exists('cp_directory_cat_columns')) {
    function cp_directory_cat_columns($cols) {
        ...
        foreach($cats as $cat) {
            ...
            $subcats = wp_list_categories('taxonomy='.APP_TAX_CAT.'&orderby=name&order=asc&hierarchical=0&show_count=1&pad_counts=1&use_desc_for_title=0&hide_empty=0&depth=1&number='.get_option('cp_dir_sub_num').'&title_li=&echo=0&child_of='.$cat->cat_ID);
...
function cp_custom_search_fields() {
    ...
    $sql = "SELECT field_name FROM ". $wpdb->prefix . "cp_ad_fields p WHERE p.field_name LIKE 'cp_%' ";
...
function custom_search_join($join) {
	...
	if ( is_search() && isset($_GET['s']) ) {
		...
		$join .= " AND (x.taxonomy = '".APP_TAX_TAG."' OR x.taxonomy = '".APP_TAX_CAT."') ";
...
function cp_dashboard_charts() {
	...
	$sql = "SELECT SUM(mc_gross) as total, payment_date FROM ".$wpdb->prefix."cp_order_info WHERE payment_status <> 'pending' AND payment_date > '" . date('Y-m-d', strtotime('-30 days')) . "' GROUP BY DATE(payment_date) DESC";
	$results = $wpdb->get_results($sql);
...
function appthemes_default_menu ($args = array()) {
	...
	$menu = $wrapperOpen . wp_list_pages('echo=0&sort_column=menu_order&depth=0&title_li=0&exclude=844,'.$excludePages) . $wrapperClose;
...
function get_pack($theID, $type = '', $return = '') {
	...
	if ( $theID > 0 )
		$the_pack = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cp_ad_packs WHERE pack_id = '.$theID.'' );
...
add_action( 'init', 'cp_fix_facebook_bug' );
...
function cp_get_geocode( $post_id, $cat = '' ) {
	...
	if ( $cat )
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT lat, lng FROM %s WHERE post_id = %d AND category = %s LIMIT 1", $wpdb->prefix . 'cp_ad_geocodes', $post_id, $cat ) );
	else
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT lat, lng FROM %s WHERE post_id = %d LIMIT 1", $wpdb->prefix . 'cp_ad_geocodes', $post_id ) );
...
function cp_do_update_geocode( $meta_id, $post_id, $meta_key, $meta_value ) {
	global $wpdb;
	if ( in_array( $meta_key, array( 'cp_city', 'cp_country', 'cp_state', 'cp_street' ) ) ) {
