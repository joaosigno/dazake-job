...
if ( !function_exists('dk_book_add_fun') ) {
/**
 ...
 */
    function dk_book_add_fun() {
        ...
        if ( !empty($_GET['error']) ) {
		 ...
		 echo '
				<div id="message" class="error fade">
					<p><strong>' . __("Succed adding book!", NRTD) . '</strong></p>
				</div>
				';
	}
        }
        ...
        if ( !empty($_GET['added']) ) {
			...
			<div id="message" class="updated fade">
				...
				<div id="dk-navi-manage">
					<a href="' . apply_filters('book_edit_url', get_page_link(intval($_GET['page_id'])) . '?ig=ig&dkaction=manage&action=editsingle&id=' . intval($_GET['added'])) . '">' . __("Edit this book") . ' &raquo;</a>
					</div>
			</div>
			';
        }
            ...
            if ( is_wp_error($results) ) {
                ...
                if ( !$results ) {
                    if ( $using_isbn )
                        ...
                        echo '<div class="error"><p>' . sprintf(__("Sorry, but amazon%s did not return any results for &ldquo;%s&rdquo;", NRTD), $options['domain'], $title) . '</p></div>';
                } else {
                    if ( $using_isbn )
                    ...
                    foreach ( (array) $results as $result ) {
                        ...
                        echo '
						<form method="post" action="' . get_option('siteurl') . '/wp-content/plugins/dk_books/admin/add.php" style="border:1px solid #ccc; padding:5px; margin:5px;">
						';
                        ...
                        echo '
							...
							') . '
							<p style="clear:left;"><input class="eMember_button" type="submit" value="' . __("Add to bookshelf", NRTD) . '" /></p>
        ...
        echo '
			...
			<p><input class="eMember_button" type="submit" value="' . __("Search", NRTD) . '" /></p>
