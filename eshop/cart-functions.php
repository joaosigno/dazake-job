...
*/
			...
			if($taxtotal != 0){
				$echo.= "<tr class=\"stotal\"><th id=\"subtotal$iswidget\" class=\"leftb\">GST(10%)</th><td headers=\"subtotal$iswidget cartTotal$iswidget\" class=\"amts lb\" colspan=\"2\">"."$".$taxtotal."</td></tr>\n";
			}
			...
			if($pzone!='' || ('yes' == $eshopoptions['downloads_only'] && isset($etax['unknown']) && $etax['unknown']!='')){
				...
				$echo.= '<tr class="total"><th id="cTotal'.$iswidget.'" class="leftb">'.__('Total Order Charges','eshop')."</th>\n<td headers=\"cTotal$iswidget cartTotal$iswidget\"  colspan=\"2\" class = \"amts lb\"><strong>"."$".($final_price + $taxtotal)."</strong></td>";
	...
	*/
	function eshop_rtn_order_details($checkid){
		...
		foreach($result as $myrow){
			...
			if(strpos(trim($itemid),'Shipping') || trim($itemid)=='postage' || trim($itemid)==__('Shipping','eshop')){
		...
		$shipid=html_entity_decode($shipid);
		$lastname=html_entity_decode($lastname);
		$array=array("status"=>$status,"firstname"=>$firstname, "lastname"=>$lastname, "ename"=>$ename,"eemail"=>$eemail,"cart"=>$cart,"downloads"=>$downloads,
		"address"=>$address,"extras"=>$extras, "contact"=>$contact,"date"=>$edited,"affiliate"=>$affiliate,"user_id"=>$user_id,"shipid"=>$shipid,
...
if (!function_exists('eshop_email_parse')) {
	...
	//	require_once ( ABSPATH . WPINC . '/registration.php' );
		...
		$this_email = str_replace('{SHIPPINGID}', $array['shipid'], $this_email);
		$this_email = str_replace('{LASTNAME}', $array['lastname'], $this_email);
