		...
		if($max>0){
			...
			foreach($myrowres as $myrow){
				...
				//if($x>0){
					...
					'<td headers=Shipping id"'.$c.'"><label for="shippingid'.$c.'">Shipping #'.$c.'</label><input type="text" value="'.$myrow->shipid.'" name="shipping['.$checkid.']" id="shipping'.$c.'" />'.
					'<td headers="bulk numb'.$c.'"><label for="move'.$c.'">Move #'.$c.'</label><input type="checkbox" value="'.$checkid.'" name="move[]" id="move'.$c.'" /></td>'.
					"</td></tr>\n";
				//}
			...
			//moved order status box
				...
				<input type="submit" id="submit1" name = "changesub" value="<?php _e('Change','eshop'); ?>" /></p>
				</fieldset>
				...
				<div class = "submit">
					<fieldset class="dazake">
						<legend><?php _e('Invoice','eshop'); ?></legend>
						<input type="hidden" name="action" value="<?php echo $_GET['action']; ?>" />
						<input type="hidden" name="generateinvoice" value="yes" />
						<input type="submit" id="invoice" name="invoicesub" value="<?php _e('Generate','eshop'); ?>" /></p>
					</fieldset>
				<!--endgenerate invoice-->

				<!--updat shipping id-->
					<fieldset class="dazake">
						<legend><?php _e('Shipping Id','eshop'); ?></legend>
						<input type="hidden" name="action" value="<?php echo $_GET['action']; ?>" />
						<input type="hidden" name="shippingid" value="yes" />
						<input type="submit" id="shipping"  name = "shipsub" value="<?php _e('Update','eshop'); ?>" /></p>
					</fieldset>
				<!--endupdat shipping id-->

				<!--sentemail -->
					<fieldset class="dazake">
						<legend><?php _e('Email','eshop'); ?></legend>
						<input type="hidden" name="action" value="<?php echo $_GET['action']; ?>" />
						<input type="hidden" name="sentemail" value="yes" />
						<input type="submit" id="sentemail"  name = "emailsub" value="<?php _e('Send','eshop'); ?>" /></p>
					</fieldset>
				<!--end sent email-->

				<!--export csv -->
					<fieldset class="dazake">
						<legend><?php _e('Csv','eshop'); ?></legend>
						<input type="hidden" name="action" value="<?php echo $_GET['action']; ?>" />
						<input type="hidden" name="csvexport" value="yes" />
						<input type="submit" id="csvexport"  name = "csvsub" value="<?php _e('Generate','eshop'); ?>" /></p>
					</fieldset>
				<!--end export csv-->
				</div>
				<?php
...
if(isset($_POST['change']) && isset($_POST['changesub'])){
...
// invoice
if(isset($_POST['generateinvoice']) && isset($_POST['invoicesub'])){
	if(isset($_POST['move']) && $_POST['move'][0]!=''){
		$_SESSION['invoice'] = '';
		echo '<div id = "invoicegenerate" ><table>';
		echo '<tr>
			<td>#</td>
		...
		foreach($_POST['move'] as $v=>$ch){
			...
			$arr = array();
			$arr['transid'] = $resultd[0]['transid'];
			$arr['edited'] = $resultd[0]['edited'];
			$arr['address'] = $resultd[0]['address1'];
				if($resultd['address2'] != '')
					$arr['address'] .=$resultd[0]['address2'];
			$arr['city'] = $resultd[0]['city'];

			$arr['state'] = $resultd[0]['state'];
			$qcode=$wpdb->escape($resultd[0]['state']);
			...
			$arr['zip'] = $resultd[0]['zip'];
			$arr['ship']  = array() ;
			$arr['ship'][0] = 0;

			$customname = '';
			if(!empty($resultd[0]['first_name'] ))
				$customname = $resultd[0]['first_name'];
			if(!empty($resultd[0]['last_name'] ))
				$customname .= ' ' .$resultd[0]['last_name'];
			if(!empty($resultd[0]['company'] ))
				$customname .= ' of '.$resultd[0]['company'];

			$countp = 0;
			$counts = 0;
			foreach ($resulti as $key => $value) {
				if( isset($value['optname'] ) &&  $value['optname'] != ''){
					$arr['product'][$countp] = array();
					$arr['product'][$countp]['optname'] = $value['optname'] ;
					$arr['product'][$countp]['item_qty'] = $value['item_qty'] ;
					$arr['product'][$countp]['item_amt'] = $value['item_amt'] ;
					$arr['product'][$countp]['tax_rate'] = $value['tax_rate'] ;
					$arr['product'][$countp]['tax_amt'] = $value['tax_amt'] ;
					$countp ++;
				}else{
					$arr['ship'][$counts]  = array() ;
					$arr['ship'][$counts] = $value['item_amt'] ;
					$counts ++;
				}
			}

			$_SESSION['invoice'][$ch] = $arr;
			$num = $v +1 ;
			$michealurl =  plugin_dir_url( __FILE__ ) . 'micheal-invoice.php';
			echo '<tr>
			<td>'.$num.'</td>
			<td>'.$resultd[0]['transid'].'</td>
			<td>'.$customname.'</td>
			<td><a target = _blank href = '.$michealurl.'?ch='.$ch.'>check invoice </a></td>
			</tr>';
		}
		echo '</table></div>';
...
if(isset($_POST['csvexport']) && isset($_POST['csvsub'])){
	if(isset($_POST['move']) && $_POST['move'][0]!=''){
		...
		foreach($_POST['move'] as $v=>$ch){
			...
			$arr['customname'] = $customname;
			$_SESSION['invoice'][$ch] = $arr;

		}
		$michealurl =  plugin_dir_url( __FILE__ ) . 'micheal-csv.php';
		echo '<tr>
		<td><a target = _blank href = '.$michealurl.'?ch='.$ch.'>Download Csv </a></td>
		</tr>';
		echo '</table></div>';
		echo '<div class="updated fade"><p>'.__('Export Csv successfully.','eshop').'</p></div>';
	}else{
		echo '<div class="error fade"><p>'.__('No Order were selected.','eshop').'</p></div>';
	}
}
