		...
		if(isset($eshopoptions['ship_types'])){
			...
			foreach ($typearr as $k=>$type){
				...
				for($z=1;$z<=$eshopoptions['numb_shipzones'];$z++){
					...
					if(isset($dispzone[$z]))
						$echozone=$dispzone[$z];
					$eshopshiptableinner .='<th id="'.$eshopletter.$y.'" class="'.$y.'">All States</th>';
				}
				...
				$eshopshiptablehead='<p><h3 class="eshop-ship4-head">'.$eshopshiptableheadtext.'</h3></p>';
...
if (!function_exists('eshop_show_zones')) {
    ...
    function eshop_show_zones($array, $legend = '' ) { 
		...
		if(in_array('state', $array)){
			...
			$echo .='<form action="#customzone" method="post" id="checkZoneBox" class="eshop eshopzones eshopstatez"><fieldset>
			<legend>'.$legend.'</legend>
